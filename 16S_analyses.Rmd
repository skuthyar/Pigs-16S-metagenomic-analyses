...
RichnessDomCat <- ggplot(df2, aes(dom_cat, Richness, fill = dom_cat)) + 
  geom_violin(trim=FALSE) + geom_point(position="jitter", size=3) + scale_fill_manual(values = c("industrial"="#963a3a","research"="#ac6d2c","captive wild"= "#4a7398","free-ranging domestic"="#52360b","free-ranging wild"="#36651e")) +stat_summary(fun = "mean", geom = "crossbar", width = 0.5,colour = "black") + theme_bw() + theme(axis.title.x=element_blank()) + theme(legend.title=element_blank()) + theme(legend.position="none") + ylab("Richness") + theme(text = element_text(size = 15))
RichnessDomCat
...
df_relab$fecal_IgA <- as.numeric(df_relab$fecal_IgA)
df_relab$Akkermansiaceae_total <- as.numeric(df_relab$Akkermansiaceae_total)
cor.test(df_relab$Akkermansiaceae_total, df_relab$fecal_IgA, na.action=na.exclude, method="spearman", exact=FALSE)
ggplot(df_relab, aes(fecal_IgA, Akkermansiaceae_total)) + geom_point() + stat_poly_line() +
  stat_poly_eq()
...
IL6 <- IL6 + stat_summary(fun = "mean",geom = "crossbar", width = 0.5,colour = "black") + theme_bw() + theme(text = element_text(size = 20)) + theme(axis.title.x = element_blank()) + theme(legend.position = "none") + ylab("Concentration of IL-6 (pg/mL)")
IL6 
...
IL10 <- IL10 + stat_summary(fun = "mean",geom = "crossbar", width = 0.5,colour = "black") + theme(text = element_text(size = 20)) + theme(axis.title.x = element_blank()) + theme(legend.position = "none")
IL10 
...
TNFalpha <- TNFalpha + theme(legend.position = "none")+theme(text = element_text(size = 20)) + theme(axis.title.x = element_blank()) 
TNFalpha
...
Fig4 <- ggarrange(IL6, IL10, TNFalpha, fecal_iga_domcat, fecal_iga_location, cor_TNFa_IgA, cor_div_IgA, labels=c("A","B","C","D","E","F", "G"), nrow=4, ncol=2)
Fig4
ggsave("Fig4.pdf", Fig4, width=40, height=20)
...
ANCOM_gen_strict_fam <- vector(mode = "list", length = 16)
for(row in 1:16) {
...
ANCOM_env_strict_fam <- vector(mode = "list", length = 21)
for(row in 1:21) {
...
Fig2 <- ggpubr::ggarrange(ANCOM_gen_strict_Plot, ANCOM_env_strict_Plot, all_patho , labels=c("A","B","C"), nrow=1, ncol=3)
ggsave('Fig2.pdf', plot  = Fig2, width = 45, height =12)
