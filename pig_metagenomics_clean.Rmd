...
Fig2 <- ggarrange(wildpigs_KO, dompigs_KO, labels=c("A","B"))
ggsave("Fig2.pdf", Fig2, width=25, height=20)
write_tsv(res_sig, "kegg_ortholog_sig.tsv")
write_tsv(enriched_ind@result, "kegg_enrich_up.tsv")
write_tsv(enriched_rep@result, "kegg_enrich_down.tsv")
...
write_tsv(resType_sig, "kegg_ortholog_frd_ind_sig.tsv")
write_tsv(enriched_ind@result, "kegg_enrich_up_frd_ind.tsv")
write_tsv(enriched_rep@result, "kegg_enrich_down_frd_ind.tsv")
...
write_tsv(resType_sig, "kegg_ortholog_frd_res_sig.tsv")
...
write_tsv(resType_sig, "kegg_modules_frd_ind_sig.tsv")
frd_ind_mod <- read.delim("kegg_modules_frd_ind_sig.tsv")
frd_ind_mod_def <- left_join(frd_ind_mod, modules, by="Module")
write_tsv(frd_ind_mod_def, "kegg_modules_frd_ind_sig.tsv")
...
write_tsv(resType_sig, "kegg_modules_frd_res_sig.tsv")
frd_res_mod <- read.delim("kegg_modules_frd_res_sig.tsv")
frd_res_mod_def <- left_join(frd_ind_mod, modules, by="Module")
write_tsv(frd_res_mod_def, "kegg_modules_frd_res_sig.tsv")
...
unnest(CAZyme)
write_tsv(res_sig_long, "CAZy_gen_sig.tsv")
...
write_tsv(resType_sig, "CAZy_frd_ind_sig.tsv")
...
write_tsv(resType_sig, "CAZy_frd_res_sig.tsv")
...
CAZy_total_plot <- ggplot(total_CAZy, aes(dom_cat, CAZy_total, fill=dom_cat)) + geom_violin(trim=FALSE) + geom_point(position="jitter", size=3) + 
  ...
  theme(axis.title.x = element_blank()) + ylab("Transcripts per Million") + theme(text = element_text(size = 20))

total_CAZy_dom <- subset(total_CAZy, total_CAZy$dom_cat != "free-ranging wild")
ggplot(total_CAZy_dom, aes(dom_cat, CAZy_total, fill=dom_cat)) + geom_boxplot() + theme_bw()
...
vfdb_matches <- read.delim("norm_vfdb_matches.txt", header=FALSE)
colnames(vfdb_matches) <- c("VF","virulence_factor")
res_sig_match <- left_join(res_sig,vfdb_matches, by="VF")
write_tsv(res_sig_match, "vf_gen_sig.tsv")
...
write_tsv(res_sig_dom_match, "vf_frd_ind_sig.tsv")
...
res_sig_dom_match <- left_join(res_sig_dom, vfdb_matches, by="VF")
write_tsv(res_sig_dom_match, "vf_frd_res_sig.tsv")
...
total_vf_plot <- total_vf_plot + theme(text = element_text(size = 20)) + scale_y_log10() 
SuppFig4 <- ggarrange(CAZy_total_plot, total_vf_plot, labels=c("A","B")) 
ggsave("SuppFig4.pdf", SuppFig4, width=25, height=8)
...
#total AROs
total_ARO <- read_excel("total_ARO_samples.xlsx")
total_ARO$total_ARO_TPM <- as.numeric(total_ARO$total_ARO_TPM)
...
pairwise.wilcox.test(total_ARO$total_ARO_TPM, total_ARO$dom_cat,
                     p.adjust.method = "BH")
total_ARO_plot <- ggplot(total_ARO, aes(dom_cat, total_ARO_TPM, fill=dom_cat)) + geom_violin(trim=FALSE) + geom_point(position="jitter", size=4) + stat_summary(fun = "mean",geom = "crossbar", width = 0.5,colour = "black") + theme_bw() + theme(legend.position = "none") + theme(axis.title.x = element_blank())
...
total_ARO_plot <- total_ARO_plot + theme(text = element_text(size = 16)) + scale_y_log10() 
ggsave("total_ARO_plot.pdf", total_ARO_plot, width=15, height=10)
...
aro_t_long_dom <- read.csv("aro_t_long.csv")
aro_t_long_dom$dom_cat <- factor(aro_t_long_dom$dom_cat, levels=c("free-ranging wild","free-ranging domestic","industrial","research"))
aro_t_long_dom$TPM <- as.numeric(aro_t_long_dom$TPM)
aro_t_long_dom$n_nas <- ifelse(aro_t_long_dom$TPM==0,NA,aro_t_long_dom$TPM)
aro_t_long_dom$ARO = factor(aro_t_long_dom$ARO, levels=c("tet(W/N/W)","tetQ","tet(40)","tetO","tetW","lnuC","CfxA6","tet(45)","B. adolescentis rpoB mutants conferring resistance to rifampicin",
...
aro_t_long_dom_filt <- aro_t_long_dom %>%
  filter(n_nas > 50)
...
Figure4 <- ggarrange(heatmap2, total_ARO_plot, labels=c("A","B"), nrow=2, ncol=1)
ggsave("Fig4.pdf", Figure4, width=20, height=15)
